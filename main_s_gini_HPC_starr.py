...
def main_s_gini_HPC(scenario,budget, FLAG_acq_option,FLAG_ret_option, FLAG_selfret_option,FLAG_ins_option,PARA_gov_benefitratio_forlow,FLAG_load_gov_opt, FLAG_DCM, save_str):
    ...
    dd = '/work/disasters/jw/STARR/Inputs_Data/'
    # user-specified parameters
    params = pd.read_csv(dd + 'parameters_STARR.csv', index_col=0, header=None)
    ...
    for year in range(1,act_years+1):
        ...
        ho_lastEXP_fld_LR_SCENYEAR = ho_lastEXP_fld_LR_SCEN30YEAR[:, year-1]
        ho_lastEXP_fld_SFHA_SCENYEAR = ho_lastEXP_fld_SFHA_SCEN30YEAR[:, year-1]
        ho_cumEXP_fld_LR_SCENYEAR = ho_cumEXP_fld_LR_SCEN30YEAR[:, year-1]
        ho_cumEXP_fld_SFHA_SCENYEAR = ho_cumEXP_fld_SFHA_SCEN30YEAR[:, year-1]
        priceAcq_alpha = gov_centers[year-1, 0]
        priceAcq_beta = gov_centers[year-1, 1]
        aftershock = gov_centers[year-1, 2]
        priceRet_alpha = gov_centers[year-1, 3]
        priceRet_beta = gov_centers[year-1, 4]
        J = params['J']
        ...
        if FLAG_acq_option == 0: # no acq
            ...
            A_LR_2in1, A_SFHA_2in1, area_priceAcq_loadingFactor_LR, area_priceAcq_loadingFactor_SFHA, ho_priceAcq_LR, ho_priceAcq_SFHA_noLastEXP, ho_priceAcq_SFHA_withLastEXP = Func_cal_acquisition(year, priceAcq_alpha, priceAcq_beta, aftershock, budget,
                             ho_acqRecord_SFHA, stru, ho_aveLoss_fld_SFHA, ho_aveLoss_wd_SFHA,
                             ho_lastEXP_fld_SFHA_SCENYEAR, temp_index_L, 
                             hoNum_LR, zones, zoneNum_LR, zoneNum_SFHA, years, acq_param_a, acq_param_b, act_zones)
       ...
       #%%
        ...
        if FLAG_acq_option + FLAG_ret_option != 0:
            ...
            A_LR_2in1, A_SFHA_2in1, R_LR_new_12in1_withGrant, R_SFHA_new_12in1_withGrant, price_Acq_Ret_aveChosen, acquired_areaID_SFHA, RESranking, A_grant, R_grant = \
                Func_cal_ranking(year, budget, stru, A_LR_2in1, A_SFHA_2in1, R_LR_new_12in1_withGrant, R_SFHA_new_12in1_withGrant, ho_acqRecord_LR, ho_acqRecord_SFHA, priceAcq_alpha, priceAcq_beta, priceRet_alpha, priceRet_beta, area_priceAcq_loadingFactor_LR, area_priceAcq_loadingFactor_SFHA, area_priceRet_loadingFactor_LR, area_priceRet_loadingFactor_SFHA, \
        ...
        """ INSURANCE """
        if FLAG_ins_option:
            insPrice_LR = np.arange(4, 15, 0.45) # search grids
            insPrice_SFHA = np.arange(1.3, 5.4, 0.15) # search grids

...
def main_s_gini_HPC(scenario, budget, FLAG_acq_option,FLAG_ret_option, FLAG_selfret_option,FLAG_ins_option,PARA_gov_benefitratio_forlow,FLAG_load_gov_opt, FLAG_DCM, save_str):
    ...
    for year in range(1,act_years+1):
        ...
        #     # Create a new DataFrame with damage per county
        #     df = ho_zone_county.copy()
        #     df['loss'] = ho_loss
            
        #     # Sum loss by county
        #     loss_by_county = df.groupby('county_ID')['loss'].sum().reset_index()
            
        #     # Calculate each county’s share of total state loss
        #     loss_by_county['share'] = loss_by_county['loss'] / loss_by_county['loss'].sum()
            
        #     # Allocate budget proportionally
        #     loss_by_county['budget'] = loss_by_county['share'] * budget
            
        #     # Final output
        #     loss_merged = loss_by_county[['county_ID', 'budget']].copy()
        ...
        #%%
        # ''' equal budget for each county '''
        # # Assume total_budget is defined earlier (e.g., from hmgp_y1 or calculated by some trigger)
        # num_counties = census_df['county_ID'].nunique()

        # # Equal allocation across counties
        # equal_budget = budget / num_counties

        # # Create loss_merged with equal budget per county
        # loss_merged = census_df[['county_ID']].copy()
        # loss_merged['budget'] = np.full(len(loss_merged), equal_budget)
        # county_flag = True
        
        ''' per capita budget allocation across counties '''
        # Load and prepare census data
        census = pd.read_csv(dd + 'dummy_census.csv', header=None)
        census.columns = ['county_ID', 'population']
        census_df = census.copy()
        ...
        # Allocate budget per capita
        census_df['budget'] = (census_df['population'] / total_population) * budget
        
        # Final loss_merged DataFrame
        loss_merged = census_df[['county_ID', 'budget']].copy()
        county_flag = True

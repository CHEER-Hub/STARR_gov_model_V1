...
# -*- coding: utf-8 -*-
"""
Created on Wed May 22 22:01:39 2024
...
def Func_cal_hoInsure_united(lambda_X, Pr, ho_perhurrLoss_fld_X, ho_perhurrLoss_wd_X,\
                              ...
                              insTail_fld_X, insTail_wd_X, index_L, risk_theta, insurance_price_LB, deductible, hoNum_LR, hoNum_SFHA, k_LR, k_SFHA, ho_dcm, FLAG_poor):
    ...
    np.random.seed(99)
    hoNum_X = len(ho_prem_fld_X)  # num HO; 649012 or 282890
    ...
    if isinstance(index_L, list) and len(index_L) == 0:
    ...
    p_flood = np.exp(np.squeeze(insTail_fld_X) - (lambda_X + tao + 1) * np.squeeze(ho_prem_fld_X) * constx_flood + ho_dcm[0] * ho_lastEXP_fld_X_year + ho_dcm[1] * ho_cumEXP_fld_X_year)  # DCM for flood insurance
    prob_fld_X = p_flood / (p_flood + 1)

    uninsured_fld_mark[:, 0] = np.random.random(hoNum_X) > prob_fld_X  # random value <= prob, then want insurance; Otherwise, uninsured; Write in col 1.
    
    p_wind = np.exp(np.squeeze(insTail_wd_X) - (lambda_X + tao + 1) * np.squeeze(ho_prem_wd_X) * constx_wind)
    prob_wd_X = p_wind / (p_wind + 1)

    uninsured_wd_mark[:, 0] = np.random.random(hoNum_X) > prob_wd_X  # stat!
    ...
    index_X_const1_fld = priced_constx_flood > ho_dcm[2]
    uninsured_fld_mark[:, 1] = (1 - index_X_const1_fld)  # priced (L-B)>eta; num HO*1; uninsured written in col 2.
    
    index_X_const1_wd = priced_constx_wind > ho_dcm[2]
    ...
    whobuy_flood_X = (np.random.random(hoNum_X) <= prob_fld_X) * fixed_const_fld  # buy insurance
    whobuy_wind_X = (np.random.random(hoNum_X) <= prob_wd_X) * fixed_const_wd
    ...
    if index_L.size > 0:
        ...
        priced_constx_flood[index_L-1] = insurance_price_LB * constx_flood[index_L-1]  # for low income (HO index), no lambda loading factor. Only (1+tau)
        priced_constx_wind[index_L-1] = insurance_price_LB * constx_wind[index_L-1]
    ...
    return (P_sum_avehurr_fldwd_X, L_sum_buy_avehurr_fldwd_X, L_sum_nobuy_perhurr_fldwd_X, 
            ...
            P_ho_fldwd_X, B_buy_perhurr_fldwd_X,num_insured_fld,num_insured_wd,L_ho_buy_perhurr_fldwd_X, \
                whofinallypay_wd_X, whofinallypay_fld_X, uninsured_fld_mark, uninsured_wd_mark) # these for are for all income
